...
void scanTick(void)
{
    ...
    if(loopMode == LOOPMODE_SCAN)
    {
        ...
        radioSetLED(0, (scanningCount++)&32);
...
void scanStop(void)
{
    ...
    scanActivityMode = 0;
    //voicePlay(VOICE_ID_SCANNING_STOP, 0); // to be implemented : voice
    if(scanModSave)
    {
...
void scanStart(void)
{
    if(settings.squelch)
    {
        if(!ACTIVE_VFOX.isFmTuner)
        {
            ...
            scanActivityMode = 0;
            updateTimer = settings.scanUpdate ? 1 : 0;
            voicePlay(VOICE_ID_SCANNING); // to be implemented : voice
            loopMode=LOOPMODE_SCAN;
            scanPrepare();
            ultraFloor = 0;
            usFloorCount = 0;
...
void scanModeFMT(void)
{
    ...
    if(fmtScanDown)
        ACTIVE_VFO.rxFreq.value-=10000l;
    else
        ACTIVE_VFO.rxFreq.value+=10000l;
    fmtSetFreq(&ACTIVE_VFO); // to be implemented : fm radio
    displayVfoRefreshActiveNoDrawBG();
    //delay(50);
    //fmtTick(); // to be implemented : fm radio tick
    if(RXA_LED)
    {
        loopMode=0;
        channelSaveActive();
...
void scanFreqCountStart(void)
{
    if(!ACTIVE_VFOSTATE.mode)
    {
        ...
        r32Save = (bkGetReg(0x32)->value)&0xFFFE;
...
void scanFreqCountTick(void)
{
    ...
    if(!(regd->bytes.hi & 0x80))
    {
        regd->bytes.hi &= 7;
        rege = bkGetReg(0x0e);
        bkSetReg(0x32, r32Save);
        ACTIVE_VFO.rxFreq.words.hi = regd->value;        
        ACTIVE_VFO.rxFreq.words.lo = rege->value;
        freqScanVhfFilter = isFreqVhf(&(ACTIVE_VFO.rxFreq.value), 1); 
        ...
        radioSetFilters(1, freqScanVhfFilter);
        displayVfoRefreshActiveNoDrawBG();
...
{
    eepromRead((u8*)&scanPresetBuffer, SCANPRESETBASE + ((u32)scanPresetIndex*24), 24); // corrected : address
    ((u8*)&scanPresetBuffer)[23]=0;
}
...
void scanDisplayPreset(void)
{
    ...
    fontDraw6x8(getModName(scanPresetBuffer.bits.modulation+1), 4, 85, COL_VFOCHNAME);
    SPRINT_NUMBER(TXT, scanPresetBuffer.step, 3, 2, 1, 1);
    fontDraw6x8(TXT, 28, 85, COL_VFOCHNAME);
    fontDraw6x8(KHZ, 67, 85, COL_VFOCHNAME);
    memCpy2(US, TXT, 3);
    SPRINT_NUMBER(&TXT[3], scanPresetBuffer.bits.ultrascan, 2, 255, 1, 1);
    fontDraw6x8(TXT, 91, 85, COL_VFOCHNAME);
}
