...
{
    memBlank(sequence, 5);
    inputStringSource = (u32)sequence;
    dtmfSeqToString(sequence, '-');
...
void inputLongPress(const u8 key)
{
    ...
    if(inputUnits==4 && inputDecimals==5 && inputUnitsCnt<4 && inputUnitsCnt>0)
    {
        ...
        if(ch>1 && ch<252)
        {
            ...
            {
                case KEY_MENU:
                    voicePlay(VOICE_ID_SAVE_CHANNEL); // to be implemented : voice
                    voiceWait();                
                    channelMap[ch]=ACTIVE_VFO.rxFreq.value;
                    eeprom_Write((u8*)&ACTIVE_VFO, CHANNELBANKBASE+(ch<<5), 32); // to be corrected : address channel, length
                    eeprom_Commit();
                    break;
                case KEY_EXIT:
                    voicePlay(VOICE_ID_DELETE_CHANNEL); // to be implemented : voice
                    voiceWait();                
                    channelMap[ch]=0l;
                    eeprom_Write(nowt, CHANNELBANKBASE+(ch<<5), 32);  // to be corrected : address channel, length
                    eeprom_Commit();
                    break;
            }
...
void inputDtmfParseKey(const u8 key)
{
    ...
    fontDrawText(TXTEDIT, CHIPFONT[inputFont], inputX, inputY, COL_INPUT, COL_BGROUND);
}    
...
void inputStringParseKey(const u8 key)
{
    ...
    else
    {
        switch(key)
        {
            ...
            case KEY_DOWN:
                ...
                break;
            case KEY_MENU:
                memCpy2(TXTEDIT, (char*)inputStringSource, inputUnits+1);
            case KEY_EXIT:
                inputState = INPUT_CANCELLED;
                inputMode &= ~INPUTMODE_INPUT;    
                return;
    ...
    COL_BGROUND = COL_INPUT;
    fontDrawText(singleChar, CHIPFONT[inputFont], inputX+(inputDecimals<<3), inputY, COL_FWBGROUND, COL_BGROUND);
...
void inputParseKey(const u8 key)
{
    ...
    else
    {
        ...
        fontDrawText(TXT, CHIPFONT[inputFont], inputX, inputY, COL_INPUT, COL_BGROUND);
    }
