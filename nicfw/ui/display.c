...
void displayDrawMeter(const u8 sig, const u8 mode)
{
    ...
    {        
        ...
        fontDrawText(TXT, &font8x8, 4, displayGetSBAR_YPOS(), COL_SUNIT, 0);
        if(plus)
        {
            TXT[0]='+';
            SPRINT_NUMBER(&TXT[1], plus, 2, 255, 0, 1);
            fontDrawText(TXT, &font8x8, 20, displayGetSBAR_YPOS(), COL_PUNIT, 0);
        }
        else
    ...
    //remoteSuspendFill = 1; // to be implemented :: remote
    for(i=0, z=1; i<120; i+=2, z++)
    {
        ...
        {            
            ...
            ((RGBCOL*)&col)->RGB.r = i > 31 ? 31 : i;
            ((RGBCOL*)&col)->RGB.b = fmtActive ? 16 : 0; 
            ((RGBCOL*)&col)->RGB.gl = green & 0x07; // 000111
            ((RGBCOL*)&col)->RGB.gh = (green & 0x38)>>3; // 111000
        }
    ...
    //remoteSuspendFill = 0; // to be implemented :: remote
}
...
void displayDrawExNoise(const u8 value, const u8 txMod)
{
    ...
    col = txMod?COL_SMETERTXMOD:COL_SMETEREXNOISE;
    // to be implemented : remote
    //if(remoteActive)
    //{
    //    uartSendByte(104);
...
const char* displayModName(const channelXtra* vfox)
{
    if(vfox->isFmTuner) return FMT;
...
void displayRefreshGeneral(void)
{
    ...
    if(!inputMode)
    {
        ...
        if(settings.asl==3) TXT[0]='I'; else TXT[0]=' ';
        TXT[1]=0;
        fontDraw8x8(TXT, FLAGS_ASLX, ypos, COL_ASLFLAG, 0);

        TXT[0]='B';
        TXT[1]=settings.channelBank+48;
        TXT[2]=0;
        fontDraw8x8(TXT, FLAGS_BANK, ypos, COL_MENU, 0);
...
void displayRefresh(const u8 drawBG)
{
    if(scopeActive && settings.pttMode==1)
        settings.pttMode=0;
    displayVfoRefresh(!settings.activeVfo, drawBG);
    displayVfoRefresh(settings.activeVfo, drawBG);
...
{
    s16 v;
    v = val>200 ? (s8)val : (u8)val;
    if(settings.pttMode==1)
    {
        v*=100;
        v/=60;
    }
    return C16(v)->bytes.lo;
}
...
{
    if(xmitActive) return COL_VFOTX;
    if(loopMode&(LOOPMODE_SCAN | LOOPMODE_SCANMONITOR | LOOPMODE_FREQCOUNTER)) return COL_VFOSCAN;    
    return squelchOpen ? COL_VFORX : COL_VFOIDLE;
}
...
void displayVfo(const u8 vfoNumber, const u8 drawBG)
{
    ...
    if(drawBG)
        lcdFill(0, vpos, 160, (settings.pttMode==1?(VFO_HEIGHT*2)-8:VFO_HEIGHT), COL_BGROUND);
    ...
    {
        rxFreqCol = VFO_CHANGERXFREQNOTBG && isActive ? colVfo() : COL_VFORXFREQ;
        if(VFO_SMALL_RXFREQ)
        {
            SPRINT_NUMBER(TXT, vfo->rxFreq.value, 4, 5, 1, 0);
            FTXT = (char*)&TXT;
            if(VFO_LJ_FREQ)
            {
                for(i=0; i!=2; i++) {
                    if(TXT[i]==' ') { TXT[i+10]=' '; TXT[i+11]=0; FTXT++; };
                }
            }
            fontDraw8x8(FTXT, VFO_RXXPOS, vpos+DIS(VFO_RXYPOS), COL_VFORXFREQ, 0);
        }
        else
        {
    ...
    else
    {
        if(VFO_SMALL_RXFREQ)
        {
            SPRINT_NUMBER(TXT, scanFreqStart/100000, 4, 255, 1, 0);
            SPRINT_NUMBER(&TXT[6], scanFreqEnd/100000, 4, 255, 1, 1);
            TXT[4]='-';
            TXT[5]='>';
    ...
    if(isActive && loopMode==LOOPMODE_SCANMONITOR && settings.scanResume)
    {
        ...
        fontCustomSymbol(13, VFO_TXXPOS-8, vpos+DIS(VFO_TXYPOS), COL_VFORXFREQ); // BOOKMARK
        fontDraw6x8(TXT, VFO_TXXPOS, displayVfoPos(settings.activeVfo)+DIS(VFO_TXYPOS), COL_VFOTXFREQ);
    }
    else
    ...
    if(!settings.bankState[settings.channelBank].vfoState[vfoNumber].group) dimColor=1;
...
void displayFreq(const u32* freq, const u8 x, const u8 y, const u16 col, u8 decimals, const u8 dp) // freq is in 10Hz units
{
    u16 col2;
    char* mhz;
    char* khz;
    ...
    fontDrawText(mhz, &font16x16, x, y, col, COL_BGROUND); // size = 64,16

    for(i=decimals+5; i<10; i++)
        TXT[i]=32;

    fontDrawText(khz, &font8x16, khzdisplace, y, col, COL_BGROUND); // size = 40, 16
    //if(dp)
    {
        col2 = dp ? lowerColor(col) : COL_BGROUND;
